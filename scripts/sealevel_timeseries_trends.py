...
ohc_archive = archive.select(
    {'variable': 'sealevel', 'type': 'timeseries', 'time_resolution': 'monthly', "name": "AVISO ftp"})
ohc_annual = ohc_archive.read_datasets(data_dir)

ds = ohc_annual[0]

ds = ds.select_year_range(1993, final_year)
...
selectc = (x >= 2015) & (x < 2025)
xc = x[selectc]
yc = y[selectc]
trendsc = np.polyfit(xc, yc, 1)

print(f"{trendsa[0]:.2f}")
print(f"{trendsb[0]:.2f}")
print(f"{trendsc[0]:.2f}")
